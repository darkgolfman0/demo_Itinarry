...
import { Tabs, AutoComplete, Checkbox, Modal, Row, Col, Button, Card, Select, Form, Divider, Typography, Input, message, Menu, Dropdown, Table, DatePicker, Space, TimePicker, InputNumber } from 'antd';
...
class test_modal extends React.Component {
    constructor(props) {
        ...
        axios.all([request_customer, request_accom, request_poi, request_transport]).then(axios.spread(async (...response) => {
            ...
            const request_transport = await response[3];
            ...
            this.setState({
                ...
                item_transport: request_transport.data
            })
        })
        ).catch(error => {
            console.error(error);
    ...
    handleAdd = () => {
        ...
        const newData = {
            key: count_key,
            day: <Button type="primary" value={"Day" + count_day} shape="circle">D{count_day}</Button>,
            date: <DatePicker size={"small"} onChange={this.onChangeDate} />,
        };
        this.setState({
    ...
    //change date
    onChangeDate = (row, dateString) => {
        const { data_addoneday, count } = this.state;
        this.state.dataSourceOneDay.map(res => {
            const data_oneday = {
                key: res.key,
                day: res.day.props.value,
                date: moment(dateString).format(dateFormat)
            }
            this.setState({
                data_addoneday: [...data_addoneday, data_oneday],
                add_day: res.day.props.value,
                oneday_date: moment(dateString).format(dateFormat),
            })
        })
    }
    ...
    submit_save = event => {
        ...
        const formdata = {
            ...
            range_date: this.state.data_addoneday,
    ...
    handleOk = e => {
        const { count_dataSource_1, dataSource, dataActivity } = this.state;
        // check type modal
        var dataTable;
        if (this.state.type_modal === "modal_POI") {
            // dataTable = {
        ...
        this.setState({
            ...
            dataActivity: [...dataActivity, dataTable],
            count_dataSource_1: count_dataSource_1 + 1,
            visible: false,
        });
        // console.log('even click add', count_dataSource_1, dataSource)
    };
    handleCancel = e => {
        this.setState({
            visible: false,
    ...
    render() {
        ...
        const ac_checkin = this.state.data_addoneday
        // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    ...
                    <div style={{ fontSize: "12px" }}>
                        ...
                        <Row>
                            <Col span={10} offset={14} style={{ textAlign: "right", top: "2em" }}>
                                <Text strong style={{ fontSize: "18px" }}>DOCUMENT ID:QUA202101-000001</Text>
                            </Col>
                        </Row>
                        ...
                        <div>
                            <Row>
                                <Col span={10} offset={14} style={{ textAlign: "right", bottom: "2em" }}>
                                    <Text strong style={{ fontSize: "18px" }}>STATUS: NEW</Text>
                                </Col>
                            </Row>
                            <Row>
                                <Col span={2} offset={4}>
                                                                            ...
                                                                            <Row>
                                                                                ...
                                                                                <Col span={2} offset={1} style={{ bottom: "3.5em" }}>
                                                                                    <Dropdown overlay={accom_checkin_menu}>
                                                                                        <Button size={"small"} style={{ width: "auto", display: "flex" }} className="ant-dropdown-link" >{this.state.modal_accom.ac_checkin}<DownOutlined /></Button>
                                                                                    </Dropdown>
                                                                                </Col>
                                                                            ...
                                                                            <Row style={{ marginTop: "0.5em" }}>
                                                                                ...
                                                                                <Col span={2}>
                                                                                    <Dropdown overlay={pickup_menu}>
                                                                                        <Button size={"small"} style={{ width: "auto", display: "flex" }} className="ant-dropdown-link" >{this.state.modal_transport.pickup}<DownOutlined /></Button>
                                                                                    </Dropdown>
                                                                                </Col>
                                                                                <Col span={2}>
                                                                                    <TimePicker
