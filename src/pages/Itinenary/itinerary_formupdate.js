...
const EditableCell = ({
    ...
    const saveOneday = async (e) => {
        try {
            const values = await form.validateFields();
            toggleEdit();
            handleSaveOneday({ ...record, ...values });
        } catch (errInfo) {
            console.log('Save failed:', errInfo);
        }
    };
    ...
    if (editableOnday) {
        childNode = editing ? (
            ...
            >
                {/* <Input ref={inputRef} onPressEnter={save} onBlur={save} /> */}
                {/* <DatePicker size={"small"} onPressEnter={saveOneday} onBlur={saveOneday} />, */}
            </Form.Item>
        ) : (
                <div
                    className="editable-cell-value-wrap"
...
class Itinerary_Formupdate extends React.Component {
    constructor(props) {
        super();
        axios.all([request_customer, request_accom, request_poi, request_transport]).then(axios.spread(async (...response) => {
    ...
    componentDidMount = async () => {
        ...
        await axios.get(`http://13.229.16.186:5000/api/itinerary/${this.props.match.params.id}`)
            .then(res => {
                console.log("res ==>", res)
                if (res) {
                    const temp_it = res.data
                    this.setState({
                        item: temp_it
                    });
                }
    ...
    handleDelete = (key) => {
        const dataSource = [...this.state.dataActivity];
        this.setState({
            dataActivity: dataSource.filter((item) => item.key !== key),
        });
    };
    ...
    handleAdd = () => {
        ...
        this.state.dataOnDay = []

        const { count_day, count_key, dataSourceOneDay } = this.state;
        const newData = {
            key: count_key,
    ...
    onSelectDate = async (key, day) => {
        ...
        console.log(key, day)
        const { count_day, count_key, add_day, dataSourceOneDay } = this.state;
    ...
    submit_save = event => {
        ...
        const formdata = {
            ...
            range_date: this.state.dataOnDay,
    ...
    handleSave = (row) => {
        const data = row
        const newData = [...this.state.dataActivity];
        ...
        this.setState({
            dataActivity: newData,
        });
    };

    // handleSaveOneday = (row) => {
    //     const newData = [...this.state.data_addoneday];
    //     const index = newData.findIndex((item) => row.key === item.key);
    //     const item = newData[index];
    //     newData.splice(index, 1, { ...item, ...row });
    //     console.log("SAVE ==>", newData)
    //     this.setState({
    //         data_addoneday: newData,
    //     });
    // };

    handleOk = e => {
        const { count_dataSource_1, dataOnDay, data_addoneday, dataSource, dataActivity } = this.state;
        // check type modal
        var dataTable;
        var data_activity;
        if (this.state.type_modal === "modal_POI") {
        ...
        if (this.state.type_modal === "modal_Accom") {
            ...
            //     day: this.state.add_day,
            //     date: this.state.modal_accom.date_checkin,
        ...
        this.setState({
            dataActivity: [...dataActivity, data_activity],
            count_dataSource_1: count_dataSource_1 + 1,
            visible: false,
        });
    };
    handleCancel = e => {
    ...
    render() {
        ...
        const { data_activity } = this.state.item

        const data_key = dataActivity.map(row => (
            {
        ...
        this.state.dataActivity = data_key
        let sum_totalprice = this.state.dataActivity.reduce(function (prev, current) {
        ...
        const components = {
            body: {
                row: EditableRow, DragableBodyRow,
                cell: EditableCell
            },
        };
                    ...
                    <div style={{ fontSize: "12px" }}>
                        ...
                        <Row>
                            <Col span={10} offset={14} style={{ textAlign: "right", top: "2em" }}>
                                <Text strong style={{ fontSize: "18px" }}>DOCUMENT ID:{this.state.item.document_id}</Text>
                            </Col>
                        </Row>
                        ...
                        <div>
                            <Row>
                                <Col span={10} offset={14} style={{ textAlign: "right", bottom: "2em" }}>
                                    <Text strong style={{ fontSize: "18px" }}>STATUS: {this.state.item.status}</Text>
                                </Col>
                            </Row>
                            <Row>
                                <Col span={2} offset={4}>
                                ...
                                <Col span={10}>
                                    ...
                                    >
                                        <Input.Search value={this.state.item.customer} size="small" placeholder="Search Customer" />
                                    </AutoComplete>
                                </Col>
                                <Col span={7} offset={1}>
                                ...
                                <Col span={3} >
                                    <Space>
                                        ...
                                        <Dropdown overlay={adult_menu} >
                                            <Button tabIndex={3} size={"small"} style={{ width: "auto", display: "flex" }} className="ant-dropdown-link" >{this.state.item.passenger_adult}<DownOutlined /></Button>
                                        </Dropdown>
                                    </Space>
                                </Col>
                                <Col span={3} >
                                    <Space>
                                        ...
                                        <Dropdown overlay={child_menu} >
                                            <Button tabIndex={4} size={"small"} style={{ width: "auto", display: "flex" }} className="ant-dropdown-link" >{this.state.item.passenger_child}<DownOutlined /></Button>
                                        </Dropdown>
                                    </Space>
                                </Col>
                                <Col span={10} >
                                    <Space>
                                        ...
                                        <Dropdown overlay={infant_menu} >
                                            <Button tabIndex={5} size={"small"} style={{ width: "auto", display: "flex" }} className="ant-dropdown-link" >{this.state.item.passenger_infant}<DownOutlined /></Button>
                                        </Dropdown>
                                    </Space>
                                </Col>
                            </Row>
                            ...
                            <Row>
                                <Col span={7} offset={5}>
                                    <Space>
                                        ...
                                        <Input tabIndex={6} name="airline" value={this.state.item.departure_airline} onChange={this.handleInputChange} size={"small"} placeholder="" />
                                    </Space>
                                </Col>
                                <Col span={7} >
                                    <Space>
                                        <Text >ARRIVAL DATE/TIME</Text>
                                        <Datetime onChange={this.arrival_date} value={moment(this.state.item.departure_date).format("DD/MM/YYYY h:mm a")} style={{ border: '1px solid #d9d9d9' }} tabIndex={7} />
                                    </Space>
                                </Col>
                            </Row>
                            <Row>
                            ...
                            <Row>
                                <Col span={7} offset={5}>
                                    <Space>
                                        <Text >AIRLINE/FLIGHT</Text>
                                        <Input tabIndex={8} name="airline_return" value={this.state.item.return_airline} onChange={this.handleInputChange} size={"small"} placeholder="" />
                                    </Space>
                                </Col>
                                <Col span={7} >
                                    <Space>
                                        <Text >RETURN DATE/TIME</Text>
                                        <Datetime onChange={this.return_date} value={moment(this.state.item.return_date).format("DD/MM/YYYY h:mm a")} style={{ border: '1px solid #d9d9d9' }} tabIndex={9} />
                                    </Space>
                                </Col>
                            </Row>
                            <br />
                            ...
                            <div>
                                ...
                                <Row>
                                    ...
                                    <Col span={3} offset={1} >
                                        <Input name="salesperson" style={{ width: "278px" }} value={this.state.item.salesperson} onChange={this.handleInputChange} size={"small"} placeholder="" />
                                    </Col>
                                    <Col span={5} offset={13} style={{ textAlign: "right" }}>
                                        <Space>
                                            ...
                                            <InputNumber
                                                size={"small"}
                                                value={this.state.item.addition_cost}
                                                formatter={value => `฿ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                                parser={value => value.replace(/^฿\s?|(,*)/g, '')}
                                ...
                                <Row>
                                    ...
                                    <Col span={6} offset={1} >
                                        <TextArea name="remark" value={this.state.item.remark} onChange={this.handleInputChange} style={{ width: "200em" }} rows={4} />
                                    </Col>
                                    <Col span={5} offset={10} style={{ textAlign: "right" }}>
                                        <Space>
                                            ...
                                            <InputNumber
                                                size={"small"}
                                                value={this.state.item.total_cost}
                                                formatter={value => `฿ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                                parser={value => value.replace(/^฿\s?|(,*)/g, '')}
                                ...
                                <Row>
                                    <Col span={5} offset={19} style={{ textAlign: "right", bottom: "3.5em" }}>
                                        <Space>
                                            ...
                                            <InputNumber
                                                size={"small"}
                                                value={this.state.item.markup_margin}
                                ...
                                <Row>
                                    <Col span={5} offset={19} style={{ textAlign: "right", bottom: "3em" }}>
                                        <Space>
                                            ...
                                            <InputNumber
                                                size={"small"}
                                                value={this.state.item.review_summary}
                                                formatter={value => `฿ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                                parser={value => value.replace(/^฿\s?|(,*)/g, '')}
                                                disabled={true}
                                ...
                                <Row>
                                    <Col span={7} offset={17} style={{ textAlign: "right", bottom: "2.5em" }}>
                                        <Space>
                                            ...
                                            <InputNumber
                                                size={"small"}
                                                value={this.state.item.avg_price}
                                                formatter={value => `฿ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                                parser={value => value.replace(/^฿\s?|(,*)/g, '')}
                                                disabled={true}
...
export default Itinerary_Formupdate;